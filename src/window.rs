...
pub fn start(render_args_ref: Arc<render::RenderArgs>, canvas: &Arc<Mutex<Vec<u8>>>) {
    ...
    let surface = match Surface::from_data(&mut canvas[..], render_args.width, render_args.height, 3 * render_args.width, PixelFormatEnum::RGB24) {
        Ok(surface) => surface,
